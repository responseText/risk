...
class UtilityController extends Controller
{
  ...
  public function alertnotkey(Request $request)
  {
			 ...
			 select
			  d.name ,
					(
						...
						where
						discover_employee_id in
						(
							select id from  employee e where e.division_id=d.id
						)
	...
	$countid = count($myArray);


    $myTxt = implode(',',$myArray);
	if($countid > 0)
	{
            $msg = 'หน่วยงาน '.$myTxt. ' ยังไม่ได้ทำการคีย์ความเสี่ยงประจำเดือน '.$date_request.' กรุณาคีย์ความเสี่ยงเข้าสู่ระบบด้วยค่ะ' ;
               $chOne = curl_init();
               //$msg = ''
